...
int main(void)
{
	...
	drawPolys();	// 도형 그리기
	drawText1();	// 문자열 출력하기
	drawText2();	// getTextSize() 함수를 이용한 문자열 정중앙에 놓기
...
void drawText2()
{
	...
	// UCHAR 타입, 3종류.?, 하얀 화면, 640X200 크기

	const String text = "Hello, OpenCV";	// 출력할 문자열
	int fontFace = FONT_HERSHEY_TRIPLEX;	// 문자열의 폰트 종류
	double fontScale = 2.0;	// 문자열 크기
	int thickness = 1;	// 굵기

	Size sizeText = getTextSize(text, fontFace, fontScale, thickness, 0);	// 출력할 문자열이 차지할 사각형 영역의 크기를 구하여 sizeText변수에 저장
	Size sizeImg = img.size();	// 출력할 대상 영상의 크기를 sizeImg 변수에 저장
